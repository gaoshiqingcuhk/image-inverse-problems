...
# This script implements the first optimization-based denoising method in the
# project: Tikhonov regularization with a squared gradient penalty.
#
# The FFT solver below uses periodic boundary conditions. This makes the
# discrete Laplacian diagonal in the Fourier domain, so each Fourier
# coefficient can be solved independently.
...
SELECTED_LAMBDAS_FOR_GRID = [0.01, 0.05, 0.1, 0.5, 1.0]
...
def tikhonov_denoise_fft(noisy: np.ndarray, lam: float) -> np.ndarray:
    ...
    with periodic boundary conditions. For an image of shape (m, n), the
    eigenvalues of the standard periodic discrete Laplacian are

        4 - 2*cos(2*pi*k/m) - 2*cos(2*pi*l/n).
    """
...
def add_result_row(
    ...
    """Append one result row to the experiment table."""
    rows.append(
        {
            "method": method,
...
def run_experiment(clean: np.ndarray, noisy: np.ndarray) -> tuple[pd.DataFrame, dict[float, np.ndarray], np.ndarray]:
    """Run noisy, Gaussian filter, and Tikhonov denoising experiments."""
    rows: list[dict[str, float | str]] = []
    tikhonov_images = {}
    ...
    return pd.DataFrame(rows), tikhonov_images, gaussian_result
...
def save_metric_curve(
    ...
    """Save a lambda sensitivity curve with noisy and Gaussian references."""
    tikhonov_results = metrics[metrics["method"] == "tikhonov"]
    ...
    axis.plot(
        tikhonov_results["parameter_value"].astype(float),
    ...
    axis.set_title(f"Tikhonov lambda sensitivity: {ylabel}")
...
def format_title(method: str, psnr: float, ssim: float) -> str:
    """Make compact subplot titles for the visual grid."""
...
def save_visual_grid(
    ...
    """Save clean, noisy, Gaussian, and selected Tikhonov results."""
...
def main() -> None:
    ...
    metrics, tikhonov_images, gaussian_result = run_experiment(clean, noisy)

    results_path = RESULTS_DIR / "04_tikhonov_denoising_results.csv"
    ...
    save_metric_curve(
        ...
        metric_name="PSNR",
        ylabel="PSNR",
        path=FIGURES_DIR / "04_tikhonov_lambda_sensitivity_psnr.png",
    )
    save_metric_curve(
        metrics,
        metric_name="SSIM",
        ylabel="SSIM",
        path=FIGURES_DIR / "04_tikhonov_lambda_sensitivity_ssim.png",
    )
    save_visual_grid(
        clean,
        noisy,
        ...
        tikhonov_images,
        metrics,
        path=FIGURES_DIR / "04_tikhonov_visual_grid.png",
    )

    print("Tikhonov denoising experiment completed successfully.")
